...
class TestImportOrder(unittest.TestCase):
    ...
    excel = HandlerExcel(os.path.join(TESTDATA_DIR, "case_data.xlsx"), "import")
    ...
    @data(*case_data)
    def test_import_order(self, case):
        '''
        导入订单测试
        :param case:用例数据接收参数
        :return:
        ...
        import_order_url = conf.get("service", "domain") + case["url"]
        method = case["method"]
        ...
        quantity = jsonpath(eval(datas), "$..quantity")[0]
        ...
        test_data = eval(replace_data(datas))
        expected = eval(case["expected"])
        import_order_response = request(method=method, url=import_order_url, headers=import_order_header,
                                        json=test_data).json()
        try:
            self.assertEqual(expected["code"], import_order_response["code"])
            self.assertEqual(expected["msg"], import_order_response["msg"])
            self.assertEqual(expected["data"]["errorReason"], jsonpath(import_order_response, "$..errorReason")[0])
            result = "Pass"
            ...
            self.excel.back_write_excel(case["case_id"] + 1, 7, str(import_order_response))
            self.excel.back_write_excel(case["case_id"] + 1, 8, str(result))

    def tearDown(self):
        try:
            if self.put_away_product_response["code"] == "000000" and self.put_away_product_response["msg"] == "成功":
                # 下架商品
